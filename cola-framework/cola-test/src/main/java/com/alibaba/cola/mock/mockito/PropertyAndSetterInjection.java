...
import static com.alibaba.cola.mock.utils.Constants.SPRING_CGLIB_PROXY_FLAG;
...
public class PropertyAndSetterInjection extends MockInjectionStrategy {
    ...
    private static final String CGLIB_CALLBACK_PREFIX = "CGLIB$CALLBACK_";
    ...
    private Object getFieldValue(Field injectMocksField, Object injectMocksFieldOwner){
        ...
        try {
            ...
            while(value != null && value.getClass().getName().indexOf(SPRING_CGLIB_PROXY_FLAG) >0){
                for(int i=0; i<20;i++){
                    BeanPropertySetter beanProperty = new BeanPropertySetter(value, CGLIB_CALLBACK_PREFIX+i);
                    Object tempValue = beanProperty.getValue();
                    if(tempValue == null){
                        break;
                    }
                    BeanPropertySetter beanProperty2 = new BeanPropertySetter(tempValue, "target");
                    tempValue = beanProperty2.getValue();
                    if(tempValue == null){
                        continue;
                    }
                    value = tempValue;
